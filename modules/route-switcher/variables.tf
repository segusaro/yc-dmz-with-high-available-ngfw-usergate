...
variable "router_healthcheck_interval" {
  description = "Interval in seconds for checking routers status using NLB healthcheck. Changing interval to lower value than default is not recommended. If changing default values additional test is required for failure scenarios."
  type = number
  default = 60
}
